...
def run_request(question_to_ask, model_type, key, alt_key):
    ...
    elif  model_type == "gpt-3.5-turbo-instruct" :  # model_type == "text-davinci-003" or 
        # Run OpenAI Completion API
        openai.api_key = key
        response = openai.Completion.create(engine=model_type,prompt=question_to_ask,temperature=0,max_tokens=900,
                    frequency_penalty=0.0,presence_penalty=0.0) #top_p=2.0 , stop=["plt.show()"]
